SELECT '' commercial_accounts'' as accounttype, COUNT(distinct account.accountid) as accounts
FROM {{ ref(''stg_salesforce_account'') }} as account
...
SELECT '' emerging_accounts'' as accounttype, COUNT(distinct account.accountid) as accounts
FROM {{ ref(''stg_salesforce_account'') }} as account
    --dbt_vidyard_master.stg_salesforce_account as account
WHERE
    account.accounttype LIKE '' Customer ''
