...
--  , nvl(zarr,0)/12 as zuoramrr
  , case when sarr > 0 and yearmonth < '2020-12' then sarr when zarr > 0 then zarr else 0 end as sarr
  , case when sarr > 0 and yearmonth < '2020-12' then sarr/12 when zarr > 0 then zarr/12 else 0 end as sarr
from zuora
